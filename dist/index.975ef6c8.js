...
function defineNota(cordaAtual, casaAtual) {
    ...
    if (range > 1) {
        ...
        notaInicial = notas[notas.findIndex((el)=>el == notaAfinacao) + range - 1 < notas.length ? notas.findIndex((el)=>el == notaAfinacao) + range - 1 : notas.findIndex((el)=>el == notaAfinacao) + range - 1 - notas.length];
    }
...
function atualizaNotas() {
    for(let i = 1; i < 6; i++)for(let j = 1; j < 6; j++){
        ...
        if (i === 5) arrayNotas[5][j - 1].replaceChild(document.createTextNode(defineNota(6, j)), arrayNotas[5][j - 1].childNodes[0]);
    }
}
